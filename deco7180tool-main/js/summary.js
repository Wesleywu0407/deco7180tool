...
function renderStats(metrics) {
  ...
  document.getElementById('section-stats').innerHTML = cards.map((c) => `
    <div class="summary-stat-card" style="--stat-accent:${c.accent};--stat-icon-bg:${c.iconBg}">
      <div class="summary-stat-accent"></div>
      <div class="summary-stat-body">
        <div class="summary-stat-icon">
          ${c.icon}
        </div>
        <div class="summary-stat-number">${c.num}</div>
        <div class="summary-stat-label">${c.label}</div>
        <div class="summary-stat-delta" style="color:${c.deltaColor}">${c.delta}</div>
...
function renderStudentRows(metrics) {
  const sorted = [...metrics.stats].sort((a, b) => b.items - a.items)
  const maxItems = sorted.reduce((max, stat) => Math.max(max, stat.items), 1)
  ...
  document.getElementById('section-students').innerHTML = sorted.map((stat) => {
    ...
    const needTags = student.needs.map((need) => `
      <span class="student-adjustment-tag" style="background:${need.bg};color:${need.text}">
        ${need.label}
      </span>
    `).join('')
    ...
    const lessonNames = metrics.lessonSummaries
      ...
      .join(' / ')
    const isExpanded = expandedStudentIds.has(stat.studentId)
    const widthPct = Math.max(10, Math.round((stat.items / maxItems) * 100))

    return `
      <button class="student-adjustment-card${isExpanded ? ' expanded' : ''}" type="button" onclick="toggleStudentSummaryDetail('${stat.studentId}')">
        <div class="student-adjustment-main">
          <div class="student-adjustment-identity">
            <div class="student-adjustment-avatar" style="background:${student.avatarBg}">
              ${student.initials}
            </div>
            <div class="student-adjustment-copy">
              <div class="student-adjustment-name">${student.name}</div>
              <div class="student-adjustment-tags">${needTags}</div>
            </div>
          </div>

          <div class="student-adjustment-metrics">
            <span><strong>${stat.items}</strong> items</span>
            <span><strong>${stat.lessons}</strong> ${stat.lessons === 1 ? 'lesson' : 'lessons'}</span>
          </div>
        </div>

        <div class="student-adjustment-progress">
          <div class="student-adjustment-progress-top">
            <span>${stat.items} of ${maxItems} adjustments</span>
            <span>${stat.pct}%</span>
          </div>
          <div class="student-adjustment-track">
            <div class="student-adjustment-fill" style="width:${widthPct}%;background:${student.avatarBg}"></div>
          </div>
        </div>

        <div class="student-adjustment-detail" aria-hidden="${isExpanded ? 'false' : 'true'}">
